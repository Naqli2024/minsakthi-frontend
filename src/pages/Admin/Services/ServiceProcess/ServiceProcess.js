...
import NewServiceProcess from "./NewServiceProcess";
import { useDispatch } from "react-redux";
import { getAllOrders } from "../../../../redux/User/BookingSlice";
import { toast } from "react-toastify";
import ViewServiceProcessDetails from "./ViewServiceProcessDetails";

const ServiceProcess = () => {
  const [openNewProcess, setOpenNewProcess] = useState(false);
  const [openViewServiceProcessDetail, setOpenViewServiceProcessDetail] = useState(false);
    ...
    <div>
      ...
      : (
        <div className="admin-pl-overall-container">
          <h1>Service Process List</h1>
          <p className="text-muted fw-bold">
            Service Process: {validOrders.length}
          </p>
                ...
                <TableBody>
                  {finalFilteredData.length > 0 ? (
                    finalFilteredData.map((order, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          <div className="id-link" onClick={()=>handleOpenProcessDetail(order)}>
                            {order.orderId || "N/A"}
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {order.serviceRequiredDate
                            ? new Date(order.serviceRequiredDate).toLocaleDateString()
                            : "N/A"}
                        </TableCell>
